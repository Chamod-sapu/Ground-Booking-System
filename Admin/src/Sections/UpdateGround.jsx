...
function UpdateGround() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.put(`${backendUrl}/Ground/update/${gId}`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
            alert('Ground Updated');
            navigate('/grounds');
        } catch (err) {
            alert(err);
        }
    ...
    return (
        <>
            <h1 className='text-3xl pl-16 py-7 font-bold text-[#000235] mt-16'>Update Ground:</h1>
            <div className=''>
                <form action="" className='border border-[#000235] rounded-xl w-[830px] ml-[290px] mt-3 p-4'>
                    ...
                    <div className='border rounded border-slate-300 w-52 mt-6 ml-[168px] text-[#000235]'>
                        ...
                        <div>
                            <button onClick={handleSubmit} className='bg-[#000235] text-white w-32 rounded-xl ml-[400px]'>Update</button>
                        </div>
                    </div>
                </form>
            </div>
